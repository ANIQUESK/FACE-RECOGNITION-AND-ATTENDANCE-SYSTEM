...
class Attendance:
    def __init__(self, root):
        ...
        # Set minimum window size to prevent it from being too small when minimized
        ...
        # Load images
        self.load_images(screen_width, screen_height)

        # Create UI elements
        self.create_ui_elements()

    def load_images(self, screen_width, screen_height):
        # Load and resize images
        img = Image.open(r"C:\Users\ANIQUE\Documents\Python_Test_Projects\Images_GUI\banner.jpg")
        img = img.resize((screen_width, 150), Image.Resampling.LANCZOS)
        self.photoimg = ImageTk.PhotoImage(img)

        bg1 = Image.open(r"Images_GUI\bg2.jpg")
        bg1 = bg1.resize((screen_width, screen_height - 150), Image.Resampling.LANCZOS)
        self.photobg1 = ImageTk.PhotoImage(bg1)

    def create_ui_elements(self):
        # Set image as label
        f_lb1 = Label(self.root, image=self.photoimg)
        f_lb1.place(x=0, y=0, width=self.root.winfo_screenwidth(), height=150)

        bg_img = Label(self.root, image=self.photobg1)
        bg_img.place(x=0, y=150, width=self.root.winfo_screenwidth(), height=self.root.winfo_screenheight() - 150)

        # Title section
        title_lb1 = Label(bg_img, text="Welcome to Attendance Panel", font=("verdana", 30, "bold"), bg="white", fg="navyblue")
        title_lb1.place(x=0, y=0, width=self.root.winfo_screenwidth(), height=50)
        ...
        # Create text boxes, combo boxes, and buttons
        self.create_left_frame_ui(left_frame)
        self.create_right_frame_ui(right_frame)

    def create_left_frame_ui(self, left_frame):
        # Student ID
    ...
    def create_right_frame_ui(self, right_frame):
        # Table Frame
        ...
        delete_btn = Button(right_frame, command=self.delete_data, text="Delete", width=12, font=("verdana", 12, "bold"), fg="white", bg="navyblue")
    ...
    def update_data(self):
        if self.var_id.get() == "" or self.var_roll.get() == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                if Update:
                    with mysql.connector.connect(username='root', password='admin', host='localhost', database='face_recognition', port=3306) as conn:
